...
- [ ] 1. Add Dispatch Unit Identification Functions
  - [ ] 1.1 Implement `is_dispatch_unit()` function in spec_parser.py
  ...
  - [ ] 1.2 Write property test for dispatch unit identification
    - **Property 1: Dispatch Unit Selection**
    - **Validates: Requirements 1.1, 1.2, 1.3, 4.3**
  - [ ] 1.3 Implement `get_dispatchable_units()` function in spec_parser.py
...
- [ ] 2. Update Dispatch Payload Structure
  - [ ] 2.1 Create `DispatchPayload` and `SubtaskInfo` dataclasses in dispatch_batch.py
  ...
  - [ ] 2.2 Implement `build_dispatch_payload()` function
  ...
  - [ ] 2.3 Write property test for payload structure
    - **Property 4: Payload Structure Completeness**
    - **Validates: Requirements 5.1, 5.2, 5.3**

- [ ] 3. Modify Task Content/Prompt Generation
  - [ ] 3.1 Update `build_task_content()` to use new payload format
  ...
  - [ ] 3.2 Update `TaskConfig` to support dispatch unit format
    - Modify heredoc format to include subtask information
    - Ensure backward compatibility with standalone tasks
    - _Requirements: 5.3_

- [ ] 4. Update Ready Task Selection in dispatch_batch.py
  - [ ] 4.1 Replace `get_ready_tasks()` with `get_dispatchable_units()` call
    - Update dispatch_batch() to use new function
    - Ensure dependency expansion still works correctly
    - _Requirements: 4.1, 4.3, 4.4_
  - [ ] 4.2 Write property test for dependency expansion
    - **Property 3: Dependency Expansion**
    - **Validates: Requirements 4.1, 4.4**

- [ ] 5. Checkpoint - Core dispatch logic complete
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 6. Update Window Allocation Logic
  - [ ] 6.1 Modify window allocation to use dispatch units
...
- [ ] 9. Implement Error Handling
  ...
  - [ ] 9.2 Write property test for failure isolation
    - **Property 8: Subtask Failure Isolation**
    - **Validates: Requirements 8.2, 8.4**
  - [ ] 9.3 Implement resume logic for unblocked subtasks
    - Resume from blocked subtask, not from beginning
    - Skip already completed subtasks
    - _Requirements: 8.3_

- [ ] 10. Checkpoint - Error handling complete
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 11. Backward Compatibility
  - [ ] 11.1 Test with flat tasks.md (no hierarchy)
...
- [ ] 12. Update Exports and Documentation
  - [ ] 12.1 Update `__init__.py` exports
    ...
    - _Requirements: 7.3_
  - [ ] 12.2 Update SKILL.md documentation
