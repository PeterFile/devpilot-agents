...
func TestExecutorHelperCoverage(t *testing.T) {
	t.Run("realCmdAndProcess", func(t *testing.T) {
		...
		var echoCmd *exec.Cmd
		if runtime.GOOS == "windows" {
			echoCmd = exec.Command("cmd", "/c", "echo", "ok")
		} else {
	...
	t.Run("generateFinalOutputAndArgs", func(t *testing.T) {
		...
		if !slices.Equal(args, []string{"e", "--model", "gpt-5.2-codex", "--skip-git-repo-check", "-C", "/tmp", "--json", "task"}) {
			t.Fatalf("unexpected codex args: %+v", args)
		}
		args = buildCodexArgs(&Config{Mode: "resume", SessionID: "sess"}, "target")
		if !slices.Equal(args, []string{"e", "--model", "gpt-5.2-codex", "--skip-git-repo-check", "--json", "resume", "sess", "target"}) {
...
func TestExecutorExecuteConcurrentWithContextBranches(t *testing.T) {
	...
	t.Run("TestConcurrentTaskLoggerFailure", func(t *testing.T) {
		// Create a writable temp dir for the main logger, then flip os.TempDir() to a non-directory
		...
		notADir := filepath.Join(writable, "not-a-dir")
		if err := os.WriteFile(notADir, []byte("x"), 0o600); err != nil {
			t.Fatalf("failed to create temp file: %v", err)
		}
		_ = setTempDirEnv(t, notADir)
