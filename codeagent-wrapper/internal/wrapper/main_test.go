...
func TestRunCodexTask_WithStdin(t *testing.T) {
	...
	fake := newFakeCmd(fakeCmdConfig{
		StdoutPlan: []fakeStdoutEvent{
			{Data: `{"type":"thread.started","thread_id":"stdin-thread"}` + "\n"},
			{Data: jsonInput + "\n"},
		},
	})
	newCommandRunner = func(ctx context.Context, name string, args ...string) commandRunner {
	...
	if got := fake.StdinContents(); got != jsonInput {
		t.Fatalf("stdin = %q, want %q", got, jsonInput)
	}
}
...
func TestRunCodexProcess_WithStdin(t *testing.T) {
	...
	fake := newFakeCmd(fakeCmdConfig{
		StdoutPlan: []fakeStdoutEvent{
			{Data: `{"type":"thread.started","thread_id":"proc"}` + "\n"},
	...
	if got := fake.StdinContents(); got != jsonOutput {
		t.Fatalf("stdin = %q, want %q", got, jsonOutput)
	}
}
...
func TestRunSilentMode(t *testing.T) {
	...
	newCommandRunner = func(ctx context.Context, name string, args ...string) commandRunner {
		return newFakeCmd(fakeCmdConfig{
			StdoutPlan: []fakeStdoutEvent{
				{Data: `{"type":"thread.started","thread_id":"silent-session"}` + "\n"},
				{Data: `{"type":"item.completed","item":{"type":"agent_message","text":"quiet"}}` + "\n"},
			},
		})
	}
	codexCommand = "fake-cmd"
...
noop`)
	...
	if !strings.Contains(report.Tasks[0].Message, "full output marker") {
		t.Fatalf("output missing task message, got %q", report.Tasks[0].Message)
	}
}
...
func TestRun_CommandFails(t *testing.T) {
	...
	stdinReader = strings.NewReader("")
	isTerminalFn = func() bool { return true }
	restore := withBackend("nonexistent-command-xyz", func(cfg *Config, targetArg string) []string { return []string{} })
	defer restore()
	if code := run(); code == 0 {
...
func TestRun_CleanupHookAlwaysCalled(t *testing.T) {
	...
	restore := withBackend(createFakeCodexScript(t, "x", "ok"), buildCodexArgs)
	defer restore()
	os.Args = []string{"codeagent-wrapper", "task"}
	if exitCode := run(); exitCode != 0 {
		t.Fatalf("exit = %d, want 0", exitCode)
...
func TestParallelLogPathInSerialMode(t *testing.T) {
	...
	isTerminalFn = func() bool { return true }
	runTaskFn = func(task TaskSpec, silent bool, timeout int) TaskResult {
		return TaskResult{ExitCode: 0, Message: "ok", SessionID: "cli-session"}
	}
...
func TestRun_CLI_Success(t *testing.T) {
	...
	restore := withBackend(createFakeCodexScript(t, "cli-session", "ok"), buildCodexArgs)
	defer restore()
